...
void setup() {
  ...
  wm.setConfigPortalTimeout(5*60); // auto close configportal after n seconds
  ...
  res = wm.autoConnect("EasyLock","ifmg123EDU"); // password protected ap
...
void checkWiFi(){
  ...
  if ( modoAp ) {
    ...
    if( digitalRead(TRIGGER_PIN) == LOW ){
      ...
      if (!wm.startConfigPortal("EasyLock","ifmg123EDU")) {
        Serial.println("failed to connect or hit timeout");
        delay(3000);
        // ESP.restart();
      } else {
